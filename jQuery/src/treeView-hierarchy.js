$(() => {
  ...
  function canDrop(treeView, e) {
    ...
    const fromIndices = e.itemData
      .map((node) => getVisualIndexByNode(treeView, node));
    const targetThemselves = toNode
      && (e.itemData.some((i) => i.key === toNode.key)
  ...
  function moveNodes(items, e, toNode, treeFieldExpr) {
    ...
    nodesToMove.forEach((nodeToMove) => {
      const fromNodeContainingArray = getNodeContainingArray(
        nodeToMove,
        items,
        treeFieldExpr.items,
      );
    ...
    if (e.dropInsideItem) {
      ...
      toNode.itemData[treeFieldExpr.items]
        .splice(toIndex, 0, ...nodesToMove.map((i) => i.itemData));
    } else {
  ...
  function getVisualIndexByNode(treeView, node) {
    ...
    const nodeElement = nodeElements
      .find((n) => n.getAttribute('data-item-id') === node.key);
